...
class PortfolioManager {
   ...
   */
  getCategoriesCapitalized = () => [
    ...[...this.menu.keys()].map(category => capitalize(category)),
  ];
  /**
   ...
   */
  getCategoriesLowercase = () => [
    ...[...this.menu.keys()].map(category => category.toLowerCase()),
  ];
  /**
   ...
   */
  getSubcategoriesAtLowercase = category => [
    ...[...this.menu.get(category).keys()].map(s => s.toLowerCase()),
  ];
  /**
   ...
   */
  getSubcategoriesFrom = index => [
    ...this.menu.get([...this.menu.keys()][index]).keys(),
  ];
   ...
   */
  getPicturesAt = (category, subcategory) =>
    this.menu.get(category).get(subcategory);
  /**
   ...
   */
  getPicturesFrom = (category, subcategory) =>
    this.menu
      .get([...this.menu.keys()][category])
      .get(
        [...this.menu.get([...this.menu.keys()][category]).keys()][subcategory],
      );
   ...
   */
  getAllPicturesAt = category =>
    this.getPicturesAt(category, 'icon').filter(
      pic => pic.width !== 0 && pic.height !== 0,
    );
  /**
   ...
   */
  getAllPicturesFrom = index =>
    this.getPicturesAt(this.getCategory(index), 'icon').filter(
      pic => pic.width !== 0 && pic.height !== 0,
    );
   ...
   */
  getAllUrlsAt = category => [
    ...this.getAllPicturesAt(category).map(
      pic =>
        `${url}${replaceAll(pic.path, '/', '%2F')}%2F${replaceAll(
          pic.name,
          ' ',
          '%20',
   ...
   */
  getAllUrlsFrom = index => [
    ...this.getAllPicturesFrom(index).map(
      pic =>
        `${url}${replaceAll(pic.path, '/', '%2F')}%2F${replaceAll(
          pic.name,
          ' ',
  ...
  getAllPictures = () => {
    ...
    this.getCategories().forEach(
      category => (tmp = tmp.concat(this.getAllPicturesAt(category))),
    );
    ...
    try {
      if (this.hasPictures(query)) {
        ...
        if (raw.length === 1) {
          ...
          if (
            subcategory.toLowerCase().localeCompare('all') === 0 ||
            subcategory.toLowerCase().localeCompare('icon') === 0 ||
            _.isEmpty(subcategory)
          ) {
            return this.getAllPicturesAt(category);
          } else {
            return this.getPicturesAt(
              category,
              this.trueSubcategoryName(category, subcategory),
            );
          }
        } else {
          return [];
        ...
        filter(query.split('/'))[0].toLowerCase().localeCompare('all') === 0
      ) {
    ...
    try {
      if (this.hasPictures(query)) {
        ...
        if (raw.length === 1) {
          ...
          return {
            category: this.trueCategoryName(raw[0]),
            subcategory: this.trueSubcategoryName(
              this.trueCategoryName(raw[0]),
              raw[1],
            ),
          };
        } else {
...
class Picture {
   ...
   */
  getUrl = () =>
    `${url}${replaceAll(this.path, '/', '%2F')}%2F${replaceAll(
      this.name,
      ' ',
      '%20',
    )}?alt=media&token=${token}`;
